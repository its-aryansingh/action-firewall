...
| 4:15–4:45 | **The loop closing.** Act 5: reconcile the unpaid link — `changed=False`. Then the payment lands and reconcile again — `action_issued -> settled`, and `confirmed_test_payment_value_paise` moves from 0 to 48600. | "Issuing a link is not payment. Reconciling an unpaid link changes nothing. The confirmed figure moves only when the provider says it did — and no field on any route lets a caller assert that money moved." |
