...
export default function CatalogAndPolicyPage() {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-text">Catalog & AI Policy</h1>
        ...
        <Link
          href="/playground"
          className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-[#0C6CF2] hover:bg-primary text-white text-xs font-semibold shadow-xs transition"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-muted">Merchant AI Budget Cap</span>
            <span className="p-1.5 rounded-lg bg-primary/10 text-[#0C6CF2]">
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
            </span>
          </div>
          <div className="mt-3 text-2xl font-bold text-text font-mono">₹1,000.00</div>
          <p className="mt-1 text-xs text-muted">Weekly ceiling per agent shopper session</p>
        </div>

        {/* Guardrail 2: Bound Rail */}
        <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-muted">Closed Registered Rail</span>
          ...
          <div className="mt-3 text-lg font-bold font-mono text-text truncate">create_payment_link</div>
          <p className="mt-1 text-xs text-muted">Zero capture, refund, or payout access to buyers</p>
        </div>

        {/* Guardrail 3: Catalog Revision */}
        <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-muted">Catalog Truth State</span>
          ...
          <div className="mt-3 text-lg font-bold font-mono text-text">
            {merchant?.catalog_revision ?? "cat_rev_20260905"}
          </div>
          <p className="mt-1 text-xs text-muted">{catalog.length} verified products server-resolved</p>
        </div>
      </div>

      {/* Dual Control Authorization Matrix */}
      <section className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
        <h2 className="text-sm font-bold text-text uppercase tracking-wider mb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl border border-success/30 bg-success/10/50">
            ...
            <ul className="mt-3 space-y-2 text-xs text-success">
              <li className="flex items-center gap-2">
                <span className="font-mono font-bold text-success">&#10003;</span>
        ...
        <div className="p-5 border-b border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="w-full sm:w-64">
            <input
              ...
              className="w-full px-3 py-1.5 text-xs bg-canvas border border-border rounded-lg focus:outline-none focus:border-[#0C6CF2]"
            />
          </div>
        </div>

        {loading ? (
          <div className="py-12 text-center text-xs text-muted">Loading catalog...</div>
        ) : filteredCatalog.length === 0 ? (
          <div className="py-12 text-center text-xs text-muted">No products matching &quot;{search}&quot;.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-muted">
              <thead className="bg-canvas text-[11px] font-semibold uppercase tracking-wider text-muted border-b border-border">
                <tr>
              ...
              <tbody className="divide-y divide-border font-sans">
                {filteredCatalog.map((p) => (
                  <tr key={p.sku} className="hover:bg-canvas/80 transition">
                    <td className="py-3 px-4 font-mono font-semibold text-text">{p.sku}</td>
                    <td className="py-3 px-4 font-medium text-text">{p.name}</td>
                    <td className="py-3 px-4">
                      <span className="px-2 py-0.5 rounded bg-canvas text-text text-[10px] font-medium uppercase font-mono">
                        {p.category}
                      </span>
                    </td>
                    <td className="py-3 px-4 font-mono font-bold text-text">{inr(p.price_paise)}</td>
                    <td className="py-3 px-4">
                      {p.in_stock ? (
                    ...
                    <td className="py-3 px-4">
                      <div className="flex flex-wrap gap-1">
                        {p.tags.map((t) => (
                          <span
                            key={t}
                            className="px-1.5 py-0.5 text-[10px] rounded bg-primary/10 text-[#0C6CF2] border border-primary/20 font-mono"
                          >
                            #{t}
                          </span>
                        ))}
      ...
      <section className="bg-primary text-white rounded-2xl p-6 shadow-sm border border-slate-800">
        ...
        <p className="mt-4 text-xs text-muted border-t border-slate-800 pt-3">
